    ...
    if let Some(ref identity_client) = state.identity_client {
        ...
        match identity_client.verify_api_key(request).await {
            Ok(response) => {
                ...
                if res.valid {
                    info!("✅ API Key authorized via IAM (Role: {}) [{}us]", res.role, latency_us);
                    state.metrics.record_request(true, latency_us);
                    return Ok(next.run(req).await);
                } else {
                    warn!("🚫 API Key rejected by IAM: {} [{}us]", res.error_message, latency_us);
            ...
            Err(e) => {
                let latency_us = start.elapsed().as_micros() as u64;
                warn!("⚠️ IAM Service error: {} [{}us]. Falling back to static keys.", e, latency_us);
                state.metrics.record_request(false, latency_us);
