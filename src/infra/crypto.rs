...
impl SignatureVerifier {
    ...
    pub async fn verify_telemetry_signature(
        ...
        let public_key_hex: Option<String> = conn.get(&key).await
            .map_err(|e| anyhow!("Redis lookup failed for {}: {}", key, e))?;

        let hex_str = public_key_hex
            .ok_or_else(|| anyhow!("Public key not found in Redis for meter: {} (Key: {})", meter_id, key))?
            .trim()
            .to_string();
        ...
        let public_key_bytes = if hex_len == 64 {
            hex::decode(&hex_str).map_err(|e| anyhow!("Failed to decode hex public key for {}: {}", meter_id, e))?
        } else {
            hex_str.into_bytes()
        };

        if public_key_bytes.is_empty() {
             return Err(anyhow!("Decoded public key is empty for meter: {} (Hex length: {})", meter_id, hex_len));
        }
        ...
        let verifying_key = VerifyingKey::from_bytes(
            &public_key_bytes.try_into().map_err(|_| anyhow!("Invalid key length"))?
        )?;
        
        let is_valid = verifying_key.verify(payload, &signature).is_ok();
        
        if !is_valid {
            warn!("🚫 Ed25519 signature verification FAILED for meter: {}", meter_id);
        }
