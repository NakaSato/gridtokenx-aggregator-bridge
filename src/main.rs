...
async fn main() -> Result<()> {
    ...
    let api_gateway_grpc_url = std::env::var("API_GATEWAY_GRPC_URL")
        .unwrap_or_else(|_| api_gateway_url.clone());
    ...
    info!("🔗 API Gateway (REST): {}", api_gateway_url);
    info!("🔗 API Gateway (gRPC): {}", api_gateway_grpc_url);
    ...
    let zone_ingester = Arc::new(ingester::zone_ingester::ZoneEventIngester::new(
        &redis_url,
        &api_gateway_grpc_url,
        aggregator.clone(),
        metrics.clone(),
        num_zones,
    ).await.context("Failed to initialize ZoneEventIngester")?);

    // Lifecycle coordination
    let shutdown_token = CancellationToken::new();

    // Run zone ingester in background
    let zone_ingester_clone = zone_ingester.clone();
    let zone_shutdown = shutdown_token.clone();
    let zone_handle = tokio::spawn(async move {
        if let Err(e) = zone_ingester_clone.run(zone_shutdown).await {
