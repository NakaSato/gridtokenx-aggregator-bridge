...
impl ProtocolStack for SunSpecStack {
    async fn handle_message(
        ...
        let mode = match model.status {
            2 => aggregator_core::models::BatteryMode::Discharging, // 2 = Producing
            4 => aggregator_core::models::BatteryMode::Charging,    // 4 = Charging (for Storage)
            _ => aggregator_core::models::BatteryMode::Idle,
        };
...
mod tests {
    ...
    async fn test_sunspec_handle_valid_message() {
        ...
        if let DeviceMetrics::BatteryState { power_kw, mode, .. } = reading.metrics {
            ...
            assert_eq!(mode, aggregator_core::models::BatteryMode::Discharging);
        } else {
    ...
    async fn test_sunspec_mode_mapping() {
        let stack = SunSpecStack::new();
        let test_cases = vec![
            (2, aggregator_core::models::BatteryMode::Discharging),
            (4, aggregator_core::models::BatteryMode::Charging),
            (1, aggregator_core::models::BatteryMode::Idle),
        ];
