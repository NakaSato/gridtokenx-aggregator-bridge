...
- **An unattributable reading is counted, not just discarded.** The `meter_readings` insert resolves the owner *inside* the statement (`JOIN` the owner projection `WHERE wallet IS NOT NULL`), so telemetry from a meter with no registry row is accepted at ingest and then silently dropped at write time — no row, no error, no trace. `aggregator_readings_unattributed_total` (`crates/aggregator-persistence/src/metrics.rs`, emitted from `pg_readings`) now counts `submitted - rows_affected` per batch. Sustained non-zero ⇒ meters are streaming telemetry nobody owns; that is the signal that a fleet needs registering. Deliberately a counter and a `debug!`, not a `warn!` per batch: the condition persists until a human registers the meter, which is exactly the shape that made the mint drain log ~100 lines/sec. **Do not make ingest reject these instead** — the registry is degraded-safe by design (`DATABASE_URL` unreachable ⇒ DB tier disabled, unseeded meters stay unattributed), so rejecting on an empty lookup would turn a registry outage into permanent telemetry loss.
